...
if (isset($_POST['loginUserBtn'])) {
	...
	if (!empty($username) && !empty($password)) {
		...
		if ($loginQuery['result']) {
			...
			// Check if the user is an employee (not an admin)
			if ($isAdminStatusFromDB == 1) {
				$_SESSION['message'] = "You cannot login as an admin in the employee portal";
				$_SESSION['status'] = "400";
				header("Location: ../login.php");
				exit;
			}
...
if (isset($_POST['insertNewLeaveBtn'])) {
	...
	if (insertNewLeave($pdo, $description, $user_id, $date_start, $date_end)) {
		// Get the leave_id of the newly inserted leave
		$sql = "SELECT MAX(leave_id) as leave_id FROM leaves WHERE user_id = ?";
		...
		// Get employee name
		$sql = "SELECT first_name, last_name FROM attendance_system_users WHERE user_id = ?";
		$stmt = $pdo->prepare($sql);
		$stmt->execute([$user_id]);
		$employee = $stmt->fetch();
		...
		createLeaveNotificationForAdmins($pdo, $leave_id, $employee_name);
